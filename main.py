...
class Shape:
    ...
    def rotate(self):
        self.rot = (self.rot + 1) if self.rot < len(self.shapes[self.figure]) - 1 else 0
            ...
            for j in range(4):
                if i * 4 + j in self.shape.image():
                    if (
                            ...
                            or j + self.shape.x < 0
                            or self.field[i + self.shape.y][j + self.shape.x] > 0
                    ):
                        return True
        return False
    ...
    if game.shape is not None:
        ...
        for i in range(4):
            for j in range(4):
                p = i * 4 + j
                if p in shape_image:
                    x, y = game.x + game.zoom * (shape_x + j) + 1, game.y + game.zoom * (shape_y + i) + 1
                    pygame.draw.rect(screen, colours[game.shape.color], [x, y, game.zoom - 2, game.zoom - 2])
